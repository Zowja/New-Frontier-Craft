...
public abstract class Entity {
	...
	public void setPosition(double d, double d1, double d2) {
		...
		float f = width / 2.0F;
		float f1 = height;
		boundingBox.setBounds(d - (double) f, (d1 - (double) yOffset)
				+ (double) ySize, d2 - (double) f, d + (double) f,
				(d1 - (double) yOffset) + (double) ySize + (double) f1, d2
						+ (double) f);
	}
	...
	public boolean isOffsetPositionInLiquid(double d, double d1, double d2) {
		AxisAlignedBB axisalignedbb = boundingBox.getOffsetBoundingBox(d, d1,
				d2);
	...
	public void moveEntity(double d, double d1, double d2) {
		...
		if (flag) {
			...
			for (; d != 0.0D
					&& worldObj.getCollidingBoundingBoxes(this,
							boundingBox.getOffsetBoundingBox(d, -1D, 0.0D))
							.size() == 0; d5 = d) {
				if (d < d8 && d >= -d8) {
					d = 0.0D;
					continue;
			...
			for (; d2 != 0.0D
					&& worldObj.getCollidingBoundingBoxes(this,
							boundingBox.getOffsetBoundingBox(0.0D, -1D, d2))
							.size() == 0; d7 = d2) {
				if (d2 < d8 && d2 >= -d8) {
					d2 = 0.0D;
					continue;
		...
		List list = worldObj.getCollidingBoundingBoxes(this,
				boundingBox.addCoord(d, d1, d2));
		for (int i = 0; i < list.size(); i++) {
			d1 = ((AxisAlignedBB) list.get(i))
					.calculateYOffset(boundingBox, d1);
		}
		...
		for (int k = 0; k < list.size(); k++) {
			d2 = ((AxisAlignedBB) list.get(k))
					.calculateZOffset(boundingBox, d2);
		}
		...
		if (stepHeight > 0.0F && flag1 && (flag || ySize < 0.05F)
			...
			for (int j2 = 0; j2 < list1.size(); j2++) {
				d1 = ((AxisAlignedBB) list1.get(j2)).calculateYOffset(
						boundingBox, d1);
			}
			...
			for (int k2 = 0; k2 < list1.size(); k2++) {
				d = ((AxisAlignedBB) list1.get(k2)).calculateXOffset(
						boundingBox, d);
			}
			...
			for (int l2 = 0; l2 < list1.size(); l2++) {
				d2 = ((AxisAlignedBB) list1.get(l2)).calculateZOffset(
						boundingBox, d2);
			}
			...
			if (!field_9293_aM && d6 != d1) {
				...
				for (int i3 = 0; i3 < list1.size(); i3++) {
					d1 = ((AxisAlignedBB) list1.get(i3)).calculateYOffset(
							boundingBox, d1);
				}
		...
		if (canTriggerWalking() && !flag && ridingEntity == null) {
			distanceWalkedModified += (double) MathHelper.sqrt_double(d10 * d10
					+ d12 * d12) * 0.59999999999999998D;
		...
		boolean flag2 = isWet();
		if (worldObj.isBoundingBoxBurning(boundingBox.func_28195_e(0.001D,
				0.001D, 0.001D))) {
			dealFireDamage(1);
			if (!flag2) {
				fire++;
	...
	public boolean isWet() {
		return inWater
				|| worldObj.canBlockBeRainedOn(MathHelper.floor_double(posX),
	...
	public boolean handleWaterMovement() {
		return worldObj.handleMaterialAcceleration(
				boundingBox.expand(0.0D, -0.40000000596046448D, 0.0D)
						.func_28195_e(0.001D, 0.001D, 0.001D), Material.water,
				this) || worldObj.handleMaterialAcceleration(
				boundingBox.expand(0.0D, -0.40000000596046448D, 0.0D)
						.func_28195_e(0.001D, 0.001D, 0.001D), Material.oil,
				this);
	}
	...
	public boolean isInsideOfMaterial(Material material) {
		...
		if (l != 0 && Block.blocksList[l].blockMaterial == material) {
			float f = BlockFluid.getPercentAir(worldObj.getBlockMetadata(i, j,
					k)) - 0.1111111F;
	...
	public void writeToNBT(NBTTagCompound nbttagcompound) {
		nbttagcompound.setTag("Pos", newDoubleNBTList(new double[] { posX,
	...
	public void mountEntity(Entity entity) {
		...
		if (ridingEntity == entity) {
			ridingEntity.riddenByEntity = null;
			ridingEntity = null;
			setLocationAndAngles(entity.posX, entity.boundingBox.minY
					+ (double) entity.height, entity.posZ, rotationYaw,
					rotationPitch);
			return;
		}
		if (ridingEntity != null) {
	...
	public void setPositionAndRotation2(double d, double d1, double d2,
		...
		setRotation(f, f1);
		List list = worldObj.getCollidingBoundingBoxes(this,
				boundingBox.func_28195_e(0.03125D, 0.0D, 0.03125D));
		if (list.size() > 0) {
			double d3 = 0.0D;
			for (int j = 0; j < list.size(); j++) {
