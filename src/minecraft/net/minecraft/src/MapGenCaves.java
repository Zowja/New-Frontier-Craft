...
public class MapGenCaves extends MapGenBase {
			...
			for (int j2 = d8; !flag2 && j2 < k1; j2++) {
				for (int l2 = d10; !flag2 && l2 < i2; l2++) {
					for (int i3 = l1 + 1; !flag2 && i3 >= d9 - 1; i3--) {
						...
						if (abyte0[j3] == Block.waterMoving.blockID
								|| abyte0[j3] == Block.waterStill.blockID) {
							flag2 = true;
						}
	...
	protected void func_868_a(World world, int i, int j, int k, int l,
			...
			double d1 = (rand.nextInt(10) > 4) ? rand.nextInt(20)+rand.nextInt(15)+35 : rand.nextInt(25) + rand.nextInt(6);
			double d2 = j * 16 + rand.nextInt(16);
			int k1 = rand.nextInt(4)+8;
			for (int l1 = 0; l1 < k1; l1++) {
				float f = rand.nextFloat() * 3.141593F * 2F;
				float f2 = rand.nextFloat() * 2.0F + rand.nextFloat();
				releaseEntitySkin(k, l, abyte0, d + rand.nextInt(8), d1, d2 + rand.nextInt(8), f2, f, 0, 0, 32+rand.nextInt(8),
			...
			if(rand.nextInt(6) == 1){
				double d = i * 16 + rand.nextInt(16);
				double d1 = ((rand.nextInt(2) == 0) ? rand.nextInt(50) + 20 : rand.nextInt(120));
				double d2 = j * 16 + rand.nextInt(16);
				float f1 = ((rand.nextFloat() - 0.5F) * 2.0F) / 8F;
				float f2 = rand.nextFloat() * 2.25F + rand.nextFloat();
				int ran = 1 + rand.nextInt(4);
				for (int l1 = 0; l1 < ran; l1++){
				float f = rand.nextFloat();
				releaseEntitySkin(k, l, abyte0, d, d1, d2, f2, f, f1, 0, 0,
						rand.nextFloat() * rand.nextFloat() * 0.3D + 0.5D);}
