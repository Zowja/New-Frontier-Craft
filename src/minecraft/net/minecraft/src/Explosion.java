...
public class Explosion {
	...
	public Explosion(World world, Entity entity, double d, double d1,
			double d2, float f) {
		isFlaming = false;
	...
	public void doExplosionA() {
			...
			for (int l = 0; l < i; l++) {
				label0: for (int j1 = 0; j1 < i; j1++) {
					...
					do {
						...
						if (f1 > 0.0F) {
							destroyedBlockPositions.add(new ChunkPosition(j4,
									k4, l4));
						}
						d5 += d * (double) f2;
						d7 += d1 * (double) f2;
		...
		for (int k2 = 0; k2 < list.size(); k2++) {
			...
			if (d4 <= 1.0D) {
				...
				entity.attackEntityFrom(exploder,
						(int) (((d13 * d13 + d13) / 2D) * 8D
								* (double) explosionSize + 1.0D));
				double d14 = d13;
		...
		if (isFlaming) {
			for (int l2 = arraylist.size() - 1; l2 >= 0; l2--) {
				...
				if (l3 == 0 && Block.opaqueCubeLookup[i4]
	...
	public void doExplosionB(boolean flag) {
		...
		for (int i = arraylist.size() - 1; i >= 0; i--) {
			...
			if (i1 > 0) {
				Block.blocksList[i1].dropBlockAsItemWithChance(worldObj, j, k,
						l, worldObj.getBlockMetadata(j, k, l), 0.3F);
				worldObj.setBlockWithNotify(j, k, l, 0);
				Block.blocksList[i1].onBlockDestroyedByExplosion(worldObj, j,
						k, l);
			}
		}
