...
public class ItemRenderer {
	...
	public ItemRenderer(Minecraft minecraft) {
		...
		field_20099_f = -1;
		mc = minecraft;
		field_28131_f = new MapItemRenderer(minecraft.fontRenderer,
				minecraft.gameSettings, minecraft.renderEngine);
	}
	...
	public void renderItem(EntityLiving entityliving, ItemStack itemstack) {
		...
		if (itemstack.itemID < 256
			...
			renderBlocksInstance.renderBlockOnInventory(
	...
	public void renderItemInFirstPerson(float f) {
		float f1 = prevEquippedProgress
				+ (equippedProgress - prevEquippedProgress) * f;
		EntityPlayerSP entityplayersp = mc.thePlayer;
		float f2 = ((EntityPlayer) (entityplayersp)).prevRotationPitch
		...
		if (itemstack != null) {
			int i = Item.itemsList[itemstack.itemID]
		...
		if (itemstack != null && itemstack.itemID == Item.mapItem.shiftedIndex) {
			...
			GL11.glTranslatef(-f16 * 0.4F, MathHelper.sin(MathHelper
					.sqrt_float(f8) * 3.141593F * 2.0F) * 0.2F, -f12 * 0.2F);
			f8 = (1.0F - f2 / 45F) + 0.1F;
			if (f8 < 0.0F) {
				f8 = 0.0F;
			}
			...
			GL11.glTranslatef(0.0F,
					(0.0F * f4 - (1.0F - f1) * 1.2F - f8 * 0.5F) + 0.04F, -0.9F
							* f4);
			GL11.glRotatef(90F, 0.0F, 1.0F, 0.0F);
			GL11.glRotatef(f8 * -85F, 0.0F, 0.0F, 1.0F);
			GL11.glEnable(32826 /* GL_RESCALE_NORMAL_EXT */);
			GL11.glBindTexture(3553 /* GL_TEXTURE_2D */, mc.renderEngine
					.getTextureForDownloadableImage(mc.thePlayer.skinUrl,
							mc.thePlayer.getEntityTexture()));
			for (f12 = 0; f12 < 2; f12++) {
				f16 = f12 * 2 - 1;
				GL11.glPushMatrix();
				...
				renderplayer1.drawFirstPersonHand(mc.thePlayer.modelAlex);
				GL11.glPopMatrix();
			}
			...
			GL11.glTranslatef(-f17 * 0.4F, MathHelper.sin(MathHelper
					.sqrt_float(f9) * 3.141593F * 2.0F) * 0.2F, -f13 * 0.2F);
			GL11.glTranslatef(0.7F * f5, -0.65F * f5 - (1.0F - f1) * 0.6F,
					-0.9F * f5);
			...
			GL11.glTranslatef(-f18 * 0.3F, MathHelper.sin(MathHelper
					.sqrt_float(f10) * 3.141593F * 2.0F) * 0.4F, -f14 * 0.4F);
			GL11.glTranslatef(0.8F * f6, -0.75F * f6 - (1.0F - f1) * 0.6F,
					-0.9F * f6);
			GL11.glRotatef(45F, 0.0F, 1.0F, 0.0F);
			GL11.glEnable(32826 /* GL_RESCALE_NORMAL_EXT */);
			...
			renderplayer.drawFirstPersonHand(mc.thePlayer.modelAlex);
			GL11.glPopMatrix();
		}
		GL11.glDisable(32826 /* GL_RESCALE_NORMAL_EXT */);
	...
	public void renderOverlays(float f) {
		...
		if (mc.thePlayer.isEntityInsideOpaqueBlock()) {
			...
			if (mc.theWorld.isBlockNormalCube(j, l, i1)) {
				renderInsideOfBlock(f,
						Block.blocksList[k1].getBlockTextureFromSide(2));
			} else {
				for (int l1 = 0; l1 < 8; l1++) {
					float f1 = ((float) ((l1 >> 0) % 2) - 0.5F)
							* mc.thePlayer.width * 0.9F;
					float f2 = ((float) ((l1 >> 1) % 2) - 0.5F)
							* mc.thePlayer.height * 0.2F;
					float f3 = ((float) ((l1 >> 2) % 2) - 0.5F)
							* mc.thePlayer.width * 0.9F;
		...
		else if (mc.thePlayer.isInsideOfMaterial(Material.oil)) {
			int k = mc.renderEngine.getTexture("/misc/oil.png");
			GL11.glBindTexture(3553 /* GL_TEXTURE_2D */, k);
			renderWarpedTextureOverlay(f);
		}
