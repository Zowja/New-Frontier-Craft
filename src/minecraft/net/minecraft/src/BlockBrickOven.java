...
public class BlockBrickOven extends BlockContainer {
	...
	protected BlockBrickOven(int i, boolean flag) {
		super(i, Material.rock);
		furnaceRand = new Random();
		isActive = flag;
		blockIndexInTexture = 199;
	}

	public int idDropped(int i, Random random) {
		return NFC.BrickOvenIdle.blockID;
	}
	...
	public boolean blockActivated(World world, int i, int j, int k,
		...
		if (world.multiplayerWorld) {
			...
			TileEntityBrickOven tileentityfurnace = (TileEntityBrickOven) world
					.getBlockTileEntity(i, j, k);
			entityplayer.displayGUIBrickOven(tileentityfurnace);
			return true;
		}
	}

	public static void updateFurnaceBlockState(boolean flag, World world,
			int i, int j, int k) {
		int l = world.getBlockMetadata(i, j, k);
		TileEntity tileentity = world.getBlockTileEntity(i, j, k);
		keepFurnaceInventory = true;
		if (flag) {
			world.setBlockWithNotify(i, j, k, NFC.BrickOvenActive.blockID);
		} else {
			world.setBlockWithNotify(i, j, k, NFC.BrickOvenIdle.blockID);
		}
