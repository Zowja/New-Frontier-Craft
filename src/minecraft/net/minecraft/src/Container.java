...
public abstract class Container
{
    ...
    public ItemStack inventoryClick(int i, int j, boolean shift, EntityPlayer entityplayer)
    {
        ...
        if(j == 0 || j == 1)
        {
            ...
            if(shift)
            {
                ...
                if(itemstack1 != null)
                {
                    int clickedSize = itemstack1.stackSize;
                    itemstack = itemstack1.copy();
                    Slot clickedSlot = (Slot)slots.get(i);
                    if(clickedSlot != null && clickedSlot.getStack() != null) //Activates when shift click leaves remainder
                    {
                        int left = clickedSlot.getStack().stackSize;
                        if(left < clickedSize)
                        {
                            inventoryClick(i, j, shift, entityplayer);
                        }
                    }
                }
            } else
