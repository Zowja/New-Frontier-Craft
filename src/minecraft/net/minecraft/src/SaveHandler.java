    ...
    implements ISaveHandler
{
    ...
    public SaveHandler(File file, String s, boolean flag)
    {
    	...
    	//Accounts for structures file too now, so the normal file will be generated with it
        if(!saveDirectory.exists()){
            File structDirectory = new File(saveDirectory, "/structures/");
        	structDirectory.mkdirs();
        	File structures = new File(Minecraft.getMinecraftDir(), "/structurepacks/");
        	for(File structure : structures.listFiles()){
        		try {
					Files.copy(structure, new File(structDirectory, structure.getName()));
				} catch (IOException e) {
					e.printStackTrace();
				}
        	}
