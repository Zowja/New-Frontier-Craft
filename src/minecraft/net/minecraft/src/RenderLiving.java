...
public class RenderLiving extends Render {
	...
	public void doRenderLiving(EntityLiving entityliving, double d, double d1,
		...
		try {
			float f2 = entityliving.prevRenderYawOffset
					...
					* f1;
			float f3 = entityliving.prevRotationYaw
			...
			float f7 = entityliving.field_705_Q
					+ (entityliving.field_704_R - entityliving.field_705_Q)
					* f1;
			float f8 = entityliving.field_703_S - entityliving.field_704_R
					* (1.0F - f1);
			if (f7 > 1.0F) {
				f7 = 1.0F;
			}
			loadDownloadableImageTexture(entityliving.skinUrl,
					entityliving.getEntityTexture());
			...
			if ((j >> 24 & 0xff) > 0 || entityliving.hurtTime > 0
			...
			GL11.glDisable(32826 /* GL_RESCALE_t5EXT */);
	...
	protected void rotateCorpse(EntityLiving entityliving, float f, float f1,
		...
		if (entityliving.deathTime > 0) {
			...
			GL11.glRotatef(f3 * getDeathMaxRotation(entityliving), 0.0F, 0.0F,
					1.0F);
		}
	}
	...
	protected int getColorMultiplier(EntityLiving entityliving, float f,
			float f1) {
		return 0;
	}
	...
	protected void passSpecialRender(EntityLiving entityliving, double d,
		...
		if (Minecraft.isDebugInfoEnabled()) {
			renderLivingLabel(entityliving,
					Integer.toString(entityliving.entityId), d, d1, d2, 64);
		}
	}

	protected void renderLivingLabel(EntityLiving entityliving, String s,
			double d, double d1, double d2, int i) {
