    ...
    implements IWorldAccess
{
    ...
    public void playRecord(String s, int i, int j, int k)
    {
        if(s != null)
        {
        	//To include c418's name in the base records
        	if(s.charAt(0) == '%'){
        		mc.ingameGUI.setRecordPlayingMessage("c418 - " + s.substring(1));
                mc.sndManager.playStreaming(s.substring(1), i, j, k, 1.0F, 1.0F);
               return;
        	} else {
        		mc.ingameGUI.setRecordPlayingMessage(s);
            ...
            double d3, double d4, double d5)
    {
        ...
        if(s.equals("spore"))
        {
            mc.effectRenderer.addEffect(new EntityBlueSporeFX(worldObj, d, d1, d2, d3, d4, d5));
        }
    }
