...
public class TexturedQuad {
	...
	public TexturedQuad(PositionTextureVertex apositiontexturevertex[], int i,
			int j, int k, int l, int x, int y) {
		this(apositiontexturevertex);
		float f = 0.0015625F;
		float f1 = 0.003125F;
		apositiontexturevertex[0] = apositiontexturevertex[0]
				.setTexturePosition((float) k / (float)x - f, (float) j / (float)y + f1);
		apositiontexturevertex[1] = apositiontexturevertex[1]
				.setTexturePosition((float) i / (float)x + f, (float) j / (float)y + f1);
		apositiontexturevertex[2] = apositiontexturevertex[2]
				.setTexturePosition((float) i / (float)x + f, (float) l / (float)y - f1);
		apositiontexturevertex[3] = apositiontexturevertex[3]
				.setTexturePosition((float) k / (float)x - f, (float) l / (float)y - f1);
	}
