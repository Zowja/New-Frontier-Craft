...
public class TileEntityMobSpawner extends TileEntity {
	...
	public boolean anyPlayerInRange() {
		return worldObj.getClosestPlayer((double) xCoord + 0.5D,
				(double) yCoord + 0.5D, (double) zCoord + 0.5D, 20D) != null;
	}
		...
		if (!worldObj.multiplayerWorld) {
			...
			for (int i = 0; i < byte0; i++) {
				...
				double d8 = (double) zCoord
						+ (worldObj.rand.nextDouble() - worldObj.rand
								.nextDouble()) * 4D;
				entityliving.setLocationAndAngles(d6, d7, d8,
						worldObj.rand.nextFloat() * 360F, 0.0F);
				entityliving.negateDrop();
	...
	private void updateDelay() {
		delay = 200 + worldObj.rand.nextInt(400);
	}
