...
public abstract class GuiContainer extends GuiScreen {
	...
	public void drawScreen(int i, int j, float f) {
		...
		if (inventoryplayer.getItemStack() == null && slot != null
				&& slot.getHasStack() && slot.getStack().getItemName() != null) {
			String s = (new StringBuilder()).append("")
					.append(slot.getStack().getItemName()).toString().trim();
			if (s.length() > 0) {
				int k1 = (i - k) + 12;
				int i2 = j - l - 12;
				int j2 = fontRenderer.getStringWidth(s);
				drawGradientRect(k1 - 3, i2 - 3, k1 + j2 + 3, i2 + 8 + 3,
	...
	private void drawSlotInventory(Slot slot) {
		...
		if (itemstack == null) {
			...
			if (k >= 0) {
				...
				drawTexturedModalRect(i, j, (k % 16) * 16, (k / 16) * 16, 16,
						16);
				GL11.glEnable(2896 /* GL_LIGHTING */);
				return;
			}
	...
	protected void mouseClicked(int i, int j, int k) {
		...
		if (k == 0 || k == 1) {
			...
			if (j1 != -1) {
				...
				mc.playerController.func_27174_b(inventorySlots.windowId, j1,
						k, flag1, mc.thePlayer);
			}
		}
	}
