...
public class WorldGenTrees extends WorldGenerator {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		if (j1 != Block.grass.blockID && j1 != Block.dirt.blockID && j1 != NFC.AlphaGrass.blockID
				|| j >= 128 - l - 1) {
			return false;
		}
		boolean alpha = false;
		if(j1 == NFC.AlphaGrass.blockID){
			alpha = true;
		}
			...
			for (int k3 = i - i3; k3 <= i + i3; k3++) {
				...
				for (int i4 = k - i3; i4 <= k + i3; i4++) {
					...
					if ((Math.abs(l3) != i3 || Math.abs(j4) != i3 || random
						...
						world.setBlockAndMetadata(k3, k1, i4, Block.leaves.blockID, 4);
						}
					}
				}
