...
public abstract class GuiSlot {
	...
	public void drawScreen(int i, int j, float f) {
			...
			if (initialClickY == -1F) {
				...
				if (j >= top && j <= bottom) {
					...
					if (i >= j1 && i <= k1 && k2 >= 0 && i2 >= 0 && k2 < k) {
						boolean flag1 = k2 == selectedElement
					...
					if (i >= l && i <= i1) {
						...
						scrollMultiplier /= (float) (bottom - top - l3)
								/ (float) i3;
					} else {
				...
				amountScrolled -= ((float) j - initialClickY)
						* scrollMultiplier;
				initialClickY = j;
			}
		} else {
		...
		tessellator.addVertexWithUV(left, bottom, 0.0D, (float) left / f1,
				(float) (bottom + (int) amountScrolled) / f1);
		tessellator.addVertexWithUV(right, bottom, 0.0D, (float) right / f1,
				(float) (bottom + (int) amountScrolled) / f1);
		tessellator.addVertexWithUV(right, top, 0.0D, (float) right / f1,
				(float) (top + (int) amountScrolled) / f1);
		tessellator.addVertexWithUV(left, top, 0.0D, (float) left / f1,
				(float) (top + (int) amountScrolled) / f1);
		tessellator.draw();
		...
		for (int l2 = 0; l2 < k; l2++) {
			...
			if (field_25123_p && isSelected(l2)) {
				...
				tessellator.addVertexWithUV(k4 + 1, j3 + i4 + 1, 0.0D, 0.0D,
						1.0D);
				tessellator.addVertexWithUV(i5 - 1, j3 + i4 + 1, 0.0D, 1.0D,
						1.0D);
		...
		if (k3 > 0) {
			...
			tessellator
					.addVertexWithUV(i1 - 1, (l4 + j4) - 1, 0.0D, 1.0D, 1.0D);
	...
	private void overlayBackground(int i, int j, int k, int l) {
		...
		tessellator.addVertexWithUV(width, j, 0.0D, (float) width / f,
				(float) j / f);
		tessellator.setColorRGBA_I(0x404040, k);
		tessellator.addVertexWithUV(width, i, 0.0D, (float) width / f,
				(float) i / f);
	...
	// v NOTE: THE REASON THIS HAS BEEN ADDED HAS BEEN LOST IN TIME. Remove if they are pointless v
