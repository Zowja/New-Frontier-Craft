...
public class WorldGenLakes extends WorldGenerator {
	...
	public WorldGenLakes(int i, int j) {
		field_15235_a = i;
		grassID = j;
	}
		...
		for (int j1 = 0; j1 < 16; j1++) {
			for (int j2 = 0; j2 < 16; j2++) {
				for (int j3 = 0; j3 < 8; j3++) {
					...
					Material material = world.getBlockMaterial(i + j1, j + j3,
							k + j2);
					...
					if (j3 < 4
							&& !material.isSolid()
							&& world.getBlockId(i + j1, j + j3, k + j2) != field_15235_a) {
						return false;
					}
				}
			...
			for (int l2 = 0; l2 < 16; l2++) {
				for (int l3 = 4; l3 < 8; l3++) {
					if (aflag[(l1 * 16 + l2) * 8 + l3]
							...
							&& world.getSavedLightValue(EnumSkyBlock.Sky, i
									+ l1, j + l3, k + l2) > 0) {
						world.setBlock(i + l1, (j + l3) - 1, k + l2,
								grassID);
					}
				}
				...
				for (int i3 = 0; i3 < 16; i3++) {
					for (int i4 = 0; i4 < 8; i4++) {
						...
						if (flag1
								...
								&& world.getBlockMaterial(i + i2, j + i4,
										k + i3).isSolid()) {
							world.setBlock(i + i2, j + i4, k + i3,
									Block.stone.blockID);
						}
					}
