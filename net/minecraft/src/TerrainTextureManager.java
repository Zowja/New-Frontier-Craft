...
public class TerrainTextureManager {
	...
	public TerrainTextureManager() {
		...
		for (int i = 0; i < 256; i++) {
			if (Block.blocksList[i] != null) {
				field_1182_g[i * 3 + 0] = Block.blocksList[i]
						.getBlockTextureFromSide(1);
				field_1182_g[i * 3 + 1] = Block.blocksList[i]
						.getBlockTextureFromSide(2);
				field_1182_g[i * 3 + 2] = Block.blocksList[i]
	...
	public void func_799_a(IsoImageBuffer isoimagebuffer) {
		...
		Chunk chunk = world.getChunkFromChunkCoords(
				isoimagebuffer.field_1354_c, isoimagebuffer.field_1353_d);
		...
		for (int i1 = l - 1; i1 >= j; i1--) {
			for (int j1 = k - 1; j1 >= i; j1--) {
				...
				for (int j2 = 0; j2 < 128; j2++) {
					...
					if (block.blockMaterial == Material.water || (block.blockMaterial == Material.oil)) {
						int l2 = world.getBlockId(j1, j2 + 1, i1);
						if (l2 != 0
								&& Block.blocksList[l2].blockMaterial == Material.water || (block.blockMaterial == Material.oil)) {
							continue;
						}
					...
					if (k2 >= 0 && k2 < 160) {
						...
						if (i2 >= 0) {
							...
							if (field_1186_c[i3] <= j2) {
								field_1186_c[i3] = j2;
								field_1180_b[i3] = 0xff000000
										| (int) (field_1181_a[j4 * 3 + 0] * f5) << 16
						...
						if (i2 < 31) {
							...
							if (field_1186_c[i3 + 1] <= j2) {
								...
								field_1180_b[i3 + 1] = 0xff000000
										| (int) (field_1181_a[j4 * 3 + 0] * f6) << 16
					...
					if (i2 >= 0) {
						...
						if (field_1186_c[j3] <= j2 - 1) {
							...
							field_1180_b[j3] = 0xff000000
									| (int) (field_1181_a[l3 * 3 + 0] * f8) << 16
					...
					if (field_1186_c[j3 + 1] <= j2 - 1) {
						...
						field_1180_b[j3 + 1] = 0xff000000
								| (int) (field_1181_a[k4 * 3 + 0] * f9) << 16
								| (int) (field_1181_a[k4 * 3 + 1] * f9) << 8
								| (int) (field_1181_a[k4 * 3 + 2] * f9);
					}
				}
	...
	private void func_800_a() {
		for (int i = 0; i < 32; i++) {
			for (int j = 0; j < 160; j++) {
				...
				field_1180_b[k] = ((field_1180_b[k] & 0xfefefe) >> 1)
						+ field_1184_e[k];
				if (l < 128) {
