...
public class Block {
	...
	protected Block(int i, Material material) {
		...
		if (blocksList[i] != null) {
			throw new IllegalArgumentException((new StringBuilder())
					...
					.append(" is already occupied by ").append(blocksList[i])
					.append(" when adding ").append(this).toString());
		} else {
	...
	public void setBlockBounds(float f, float f1, float f2, float f3, float f4,
			float f5) {
		minX = f;
		minY = f1;
		minZ = f2;
	...
	public boolean getIsBlockSolid(IBlockAccess iblockaccess, int i, int j,
			int k, int l) {
	...
	public int getBlockTexture(IBlockAccess iblockaccess, int i, int j, int k,
			int l) {
		return getBlockTextureFromSideAndMetadata(l,
	...
	public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int i,
			int j, int k) {
		return AxisAlignedBB.getBoundingBoxFromPool((double) i + minX,
				(double) j + minY, (double) k + minZ, (double) i + maxX,
				(double) j + maxY, (double) k + maxZ);
	}

	public void getCollidingBoundingBoxes(World world, int i, int j, int k,
			AxisAlignedBB axisalignedbb, ArrayList arraylist) {
		AxisAlignedBB axisalignedbb1 = getCollisionBoundingBoxFromPool(world,
				i, j, k);
		if (axisalignedbb1 != null
				&& axisalignedbb.intersectsWith(axisalignedbb1)) {
	...
	public float blockStrength(EntityPlayer entityplayer) {
		...
		if (!entityplayer.canHarvestBlock(this)) {
			...
			return entityplayer.getCurrentPlayerStrVsBlock(this)
					/ blockHardness / 30F;
		}
	}
	...
	protected void dropBlockAsItem_do(World world, int i, int j, int k,
			ItemStack itemstack) {
		if (world.multiplayerWorld) {
			return;
		} else {
	...
	public MovingObjectPosition collisionRayTrace(World world, int i, int j,
		...
		Vec3D vec3d8 = null;
		if (vec3d2 != null
				&& (vec3d8 == null || vec3d.distanceTo(vec3d2) < vec3d
						.distanceTo(vec3d8))) {
			vec3d8 = vec3d2;
		}
		if (vec3d3 != null
				&& (vec3d8 == null || vec3d.distanceTo(vec3d3) < vec3d
						.distanceTo(vec3d8))) {
			vec3d8 = vec3d3;
		}
		if (vec3d4 != null
				&& (vec3d8 == null || vec3d.distanceTo(vec3d4) < vec3d
						.distanceTo(vec3d8))) {
			vec3d8 = vec3d4;
		}
		if (vec3d5 != null
				&& (vec3d8 == null || vec3d.distanceTo(vec3d5) < vec3d
						.distanceTo(vec3d8))) {
			vec3d8 = vec3d5;
		}
		if (vec3d6 != null
				&& (vec3d8 == null || vec3d.distanceTo(vec3d6) < vec3d
						.distanceTo(vec3d8))) {
			vec3d8 = vec3d6;
		}
		if (vec3d7 != null
				&& (vec3d8 == null || vec3d.distanceTo(vec3d7) < vec3d
						.distanceTo(vec3d8))) {
			vec3d8 = vec3d7;
		}
		if (vec3d8 == null) {
		...
		return new MovingObjectPosition(i, j, k, byte0, vec3d8.addVector(i, j,
				k));
	}
	...
	private boolean isVecInsideYZBounds(Vec3D vec3d) {
		if (vec3d == null) {
			...
			return vec3d.yCoord >= minY && vec3d.yCoord <= maxY
					&& vec3d.zCoord >= minZ && vec3d.zCoord <= maxZ;
		}
	}
	...
	private boolean isVecInsideXZBounds(Vec3D vec3d) {
		if (vec3d == null) {
			...
			return vec3d.xCoord >= minX && vec3d.xCoord <= maxX
					&& vec3d.zCoord >= minZ && vec3d.zCoord <= maxZ;
		}
	}
	...
	public void onBlockPlacedBy(World world, int i, int j, int k,
		...
		if(entityliving instanceof EntityPlayer){
			((EntityPlayer) entityliving).increaseScore(1);
		}
	}
	...
	static {
		...
		soundGrassFootstep = new StepSound("grass", 0.5F, 1.0F);
		...
		stone = (new BlockStone(1, 1)).setHardness(1.5F).setResistance(8F)
				.setStepSound(soundStoneFootstep).setBlockName("Stone");
		grass = (BlockGrass) (new BlockGrass(2)).setHardness(1.2F)
				.setStepSound(soundGrassFootstep).setBlockName("Grass");
		dirt = (new BlockDirt(3, 2)).setHardness(1F)
		...
		cobblestone = (new Block(4, 16, Material.rock)).setHardness(2.0F)
				...
				.setBlockName("Cobblestone");
		planks = (new Block(5, 4, Material.wood)).setHardness(2.0F)
				.setResistance(5F).setStepSound(soundWoodFootstep)
				.setBlockName("Planks").disableNeighborNotifyOnMetadataChange();
		...
		lavaMoving = (new BlockFlowing(10, Material.lava)).setHardness(0.0F)
				...
				.disableStats().disableNeighborNotifyOnMetadataChange();
		...
		oreGold = (new BlockOre(14, 32)).setHardness(3F).setResistance(500F)
				.setStepSound(soundStoneFootstep).setBlockName("Gold Ore");
		oreIron = (new BlockOre(15, 33)).setHardness(4F).setResistance(500F)
				.setStepSound(soundStoneFootstep).setBlockName("Iron Ore");
		oreCoal = (new BlockOre(16, 34)).setHardness(3F).setResistance(500F)
				.setStepSound(soundStoneFootstep).setBlockName("Coal Ore");
		wood = (new BlockLog(17)).setHardness(2.5F)
		...
		leaves = (BlockLeaves) (new BlockLeaves(18, 52)).setHardness(0.2F)
				...
				.setBlockName("Leaves").disableStats()
				.disableNeighborNotifyOnMetadataChange();
		...
		glass = (new BlockGlass(20, 49, Material.glass, false))
				...
				.setBlockName("Glass");
		oreLapis = (new BlockOre(21, 160)).setHardness(3F).setResistance(500F)
				.setStepSound(soundStoneFootstep).setBlockName("Lapis Ore");
		blockLapis = (new Block(22, 144, Material.rock)).setHardness(3F)
				.setResistance(5F).setStepSound(soundStoneFootstep)
				.setBlockName("Lapis Block");
		...
		musicBlock = (new BlockNote(25)).setHardness(0.8F)
				.setBlockName("Music Block")
				.disableNeighborNotifyOnMetadataChange();
		blockBed = (new BlockBed(26)).setHardness(0.2F).setBlockName("Bed")
				.disableStats().disableNeighborNotifyOnMetadataChange();
		railPowered = (new BlockRail(27, 179, true)).setHardness(0.7F)
				.setStepSound(soundMetalFootstep).setBlockName("Powered Rail")
				.disableNeighborNotifyOnMetadataChange();
		railDetector = (new BlockDetectorRail(28, 195)).setHardness(0.7F)
				.setStepSound(soundMetalFootstep).setBlockName("Detector Rail")
				.disableNeighborNotifyOnMetadataChange();
		pistonStickyBase = (new BlockPistonBase(29, 106, true)).setBlockName(
				"Sticky Piston").disableNeighborNotifyOnMetadataChange();
		...
		blockGold = (new BlockOreStorage(41, 23)).setHardness(2F)
				.setResistance(40F).setStepSound(soundMetalFootstep)
				.setBlockName("Gold Block");
		...
		cobblestoneMossy = (new Block(48, 36, Material.rock)).setHardness(2.0F)
				.setResistance(10F).setStepSound(soundStoneFootstep)
				.setBlockName("Mossy Cobblestone");
		obsidian = (new BlockObsidian(49, 37)).setHardness(8F)
		...
		torchWood = (new BlockTorch(50, 80)).setHardness(0.0F)
				...
				.setBlockName("Torch").disableNeighborNotifyOnMetadataChange();
		...
		mobSpawner = (new BlockMobSpawner(52, 65)).setHardness(5F)
				...
				.setBlockName("Monster Spawner").disableStats();
		stairCompactPlanks = (new BlockStairs(53, planks)).setBlockName(
				"Wooden Stairs").disableNeighborNotifyOnMetadataChange();
		...
		redstoneWire = (new BlockRedstoneWire(55, 164)).setHardness(0.0F)
				...
				.setBlockName("Redstone Dust").disableStats()
				.disableNeighborNotifyOnMetadataChange();
		oreDiamond = (new BlockOre(56, 50)).setHardness(10F).setResistance(500F)
				.setStepSound(soundStoneFootstep).setBlockName("Diamond Ore");
		blockDiamond = (new BlockOreStorage(57, 24)).setHardness(5F)
				.setResistance(320F).setStepSound(soundMetalFootstep)
				.setBlockName("Diamond Block");
		...
		signPost = (new BlockSign(63, net.minecraft.src.TileEntitySign.class,
				...
				.setBlockName("Sign").disableStats()
				.disableNeighborNotifyOnMetadataChange();
		doorWood = (new BlockDoor(64, Material.wood)).setHardness(3F)
				.setStepSound(soundWoodFootstep).setBlockName("Wooden Door")
				.disableStats().disableNeighborNotifyOnMetadataChange();
		...
		pressurePlateStone = (new BlockPressurePlate(70,
				...
				.setBlockName("Pressure Plate")
				.disableNeighborNotifyOnMetadataChange();
		doorSteel = (new BlockDoor(71, Material.iron)).setHardness(5F)
				.setStepSound(soundMetalFootstep).setBlockName("Iron Door")
				.disableStats().disableNeighborNotifyOnMetadataChange();
		pressurePlatePlanks = (new BlockPressurePlate(72,
				...
				.setStepSound(soundWoodFootstep).setBlockName("Pressure Plate")
				.disableNeighborNotifyOnMetadataChange();
		oreRedstone = (new BlockRedstoneOre(73, 51, false)).setHardness(4F)
				.setResistance(500F).setStepSound(soundStoneFootstep)
				.setBlockName("Redstone Ore")
				.disableNeighborNotifyOnMetadataChange();
		oreRedstoneGlowing = (new BlockRedstoneOre(74, 51, true))
				.setLightValue(0.625F).setHardness(4F).setResistance(500F)
				.setStepSound(soundStoneFootstep).setBlockName("Redstone Ore")
				.disableNeighborNotifyOnMetadataChange();
		torchRedstoneIdle = (new BlockRedstoneTorch(75, 115, false))
				.setHardness(0.0F).setStepSound(soundWoodFootstep)
				.setBlockName("Redstone Torch")
				.disableNeighborNotifyOnMetadataChange();
		torchRedstoneActive = (new BlockRedstoneTorch(76, 99, true))
				.setHardness(0.0F).setLightValue(0.5F)
				.setStepSound(soundWoodFootstep).setBlockName("Redstone Torch")
				.disableNeighborNotifyOnMetadataChange();
		button = (new BlockButton(77, stone.blockIndexInTexture))
				.setHardness(0.5F).setStepSound(soundStoneFootstep)
				.setBlockName("Button").disableNeighborNotifyOnMetadataChange();
		...
		netherrack = (new BlockNetherrack(87, 103)).setHardness(0.4F)
				.setStepSound(soundStoneFootstep).setBlockName("Netherrack");
		slowSand = (new BlockSoulSand(88, 104)).setHardness(0.5F)
				.setStepSound(soundSandFootstep).setBlockName("Soul Sand");
		glowStone = (new BlockGlowStone(89, 105, Material.rock))
				.setHardness(0.3F).setStepSound(soundGlassFootstep)
				.setLightValue(1.0F).setBlockName("Glowstone");
		...
		Item.itemsList[cloth.blockID] = (new ItemCloth(cloth.blockID - 256))
				.setItemName("Wool");
		Item.itemsList[glass.blockID] = (new ItemGlass(glass.blockID - 256))
				.setItemName("Glass");
