...
public abstract class EntityLiving extends Entity {
	...
	public boolean canEntityBeSeen(Entity entity) {
		return worldObj.rayTraceBlocks(Vec3D.createVector(posX, posY
				+ (double) getEyeHeight(), posZ), Vec3D.createVector(
				entity.posX, entity.posY + (double) entity.getEyeHeight(),
				entity.posZ)) == null;
	}
	...
	public void onEntityUpdate() {
		...
		if (isEntityAlive() && (isInsideOfMaterial(Material.water) || isInsideOfMaterial(Material.oil))
	...
	public void onDeath(Entity entity) {
		if (entity != null) {
			entity.addToPlayerScore(this);
		}
		if (entity != null) {
			entity.onKillEntity(this);
		}
		unused_flag = true;
		if (!worldObj.multiplayerWorld && canDrop) {
			dropFewItems();
		}
	...
	public void writeEntityToNBT(NBTTagCompound nbttagcompound) {
		...
		if(canDrop)
			nbttagcompound.setByte("CanDrop", (byte) 0);
		else 
			nbttagcompound.setByte("CanDrop", (byte) 1);
	}
	...
	public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		byte d = nbttagcompound.getByte("CanDrop");
		if(d == 0)
			canDrop = true;
		else
			canDrop = false;
	...
	protected void updatePlayerActionState() {
		...
		if (rand.nextFloat() < 0.02F) {
			EntityPlayer entityplayer1 = worldObj.getClosestPlayerToEntity(
					this, f);
	...
	public Vec3D getLook(float f) {
		if (f == 1.0F) {
			...
			float f2 = prevRotationPitch + (rotationPitch - prevRotationPitch)
					* f;
