...
public class NetClientHandler extends NetHandler {
	...
	public void handleLogin(Packet1Login packet1login) {
		...
		worldClient = new WorldClient(this, packet1login.mapSeed,
				packet1login.dimension);
	...
	public void handlePickupSpawn(Packet21PickupSpawn packet21pickupspawn) {
		...
		EntityItem entityitem = new EntityItem(worldClient, d, d1, d2,
				new ItemStack(packet21pickupspawn.itemID,
						packet21pickupspawn.count,
						packet21pickupspawn.itemDamage));
	...
	public void handleVehicleSpawn(Packet23VehicleSpawn packet23vehiclespawn) {
		...
		if (packet23vehiclespawn.type == 63) {
			obj = new EntityFireball(worldClient, d, d1, d2,
					(double) packet23vehiclespawn.field_28047_e / 8000D,
					(double) packet23vehiclespawn.field_28046_f / 8000D,
					(double) packet23vehiclespawn.field_28045_g / 8000D);
		...
		if (obj != null) {
			...
			worldClient.func_712_a(packet23vehiclespawn.entityId,
					((Entity) (obj)));
	...
	public void func_21146_a(Packet25EntityPainting packet25entitypainting) {
		EntityPainting entitypainting = new EntityPainting(worldClient,
				...
				packet25entitypainting.direction, packet25entitypainting.title);
	...
	public void func_6498_a(Packet28EntityVelocity packet28entityvelocity) {
		...
		if (entity == null) {
			...
			entity.setVelocity((double) packet28entityvelocity.motionX / 8000D,
					(double) packet28entityvelocity.motionY / 8000D,
					(double) packet28entityvelocity.motionZ / 8000D);
			return;
		}
	}
	...
	public void func_21148_a(Packet40EntityMetadata packet40entitymetadata) {
		...
		if (entity != null && packet40entitymetadata.func_21047_b() != null) {
			entity.getDataWatcher().updateWatchedObjectsFromList(
					packet40entitymetadata.func_21047_b());
		}
	}
	...
	public void handleNamedEntitySpawn(
		...
		EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(
				mc.theWorld, packet20namedentityspawn.name, packet20namedentityspawn.id);
		...
		worldClient.func_712_a(packet20namedentityspawn.entityId,
				entityotherplayermp);
	}
	...
	public void handleEntity(Packet30Entity packet30entity) {
		...
		if (entity == null) {
			...
			double d2 = (double) entity.serverPosZ / 32D;
			float f = packet30entity.rotating ? (float) (packet30entity.yaw * 360) / 256F
					: entity.rotationYaw;
			float f1 = packet30entity.rotating ? (float) (packet30entity.pitch * 360) / 256F
	...
	public void handlePreChunk(Packet50PreChunk packet50prechunk) {
		worldClient.doPreChunk(packet50prechunk.xPosition,
				packet50prechunk.yPosition, packet50prechunk.mode);
	}
	...
	public void handleMultiBlockChange(
		...
		Chunk chunk = worldClient.getChunkFromChunkCoords(
				packet52multiblockchange.xPosition,
	...
	public void handleMapChunk(Packet51MapChunk packet51mapchunk) {
		worldClient.func_711_c(packet51mapchunk.xPosition,
				packet51mapchunk.yPosition, packet51mapchunk.zPosition,
		...
		worldClient.setChunkData(packet51mapchunk.xPosition,
				packet51mapchunk.yPosition, packet51mapchunk.zPosition,
				packet51mapchunk.xSize, packet51mapchunk.ySize,
				packet51mapchunk.zSize, packet51mapchunk.chunk);
	}

	public void handleBlockChange(Packet53BlockChange packet53blockchange) {
		worldClient.func_714_c(packet53blockchange.xPosition,
				packet53blockchange.yPosition, packet53blockchange.zPosition,
				packet53blockchange.type, packet53blockchange.metadata);
	}
	...
	public void handleKickDisconnect(
		...
		mc.displayGuiScreen(new GuiConnectFailed("disconnect.disconnected",
	...
	public void handleCollect(Packet22Collect packet22collect) {
		...
		if (entity != null) {
			...
			mc.effectRenderer.addEffect(new EntityPickupFX(mc.theWorld, entity,
					(Entity) obj, -0.5F));
			worldClient
					.removeEntityFromWorld(packet22collect.collectedEntityId);
		}
	}
	...
	public void func_22186_a(Packet17Sleep packet17sleep) {
		...
		if (packet17sleep.field_22046_e == 0) {
			EntityPlayer entityplayer = (EntityPlayer) entity;
			entityplayer.sleepInBedAt(packet17sleep.field_22044_b,
					packet17sleep.field_22048_c, packet17sleep.field_22047_d);
		}
	}

	private GameProfile getGameProfile() throws IllegalArgumentException {
		try {
			return new GameProfile(UUIDTypeAdapter.fromString(mc.session.uuid),
					mc.session.username);
	...
	public void handleHandshake(Packet2Handshake var1) {
		if (var1.username.equals("-")) {
			if(mc.session.uuid.equals("0")){
				this.addToSendQueue(new Packet1Login(this.mc.session.username, null, NFC.rawVersion));
			} else
			this.addToSendQueue(new Packet1Login(this.mc.session.username, mc.session.getProfile().getId(), NFC.rawVersion));
		} else {
			try {
				// System.out.println("Attempting online log in.");
				// System.out.println("gameprofile: " +
				// getGameProfile().getName() + " " + getGameProfile().getId());
				// System.out.println("session: " + mc.session.username + " " +
				// mc.session.uuid + " " + mc.session.token + " " +
				// mc.session.getSessionID());
				mc.sessionService.joinServer(getGameProfile(),
						mc.session.token, var1.username);
			} catch (AuthenticationUnavailableException var8) {
				failConnect("Authentication servers unavailable",
						"disconnect.loginFailedInfo.serversUnavailable");
				return;
			} catch (InvalidCredentialsException var9) {
				failConnect(
						"Invalid session (try launching the game from the Mojang launcher)",
						"disconnect.loginFailedInfo.invalidSession");
				return;
			} catch (AuthenticationException var10) {
				failConnect("Failed to authenticate",
						"disconnect.loginFailedInfo");
				return;
			} catch (IllegalArgumentException e) {
				failConnect("disconnect.loginFailedInfo",
						"Invalid credentials (try launching the game from the Mojang launcher)");
				return;
			}
			this.addToSendQueue(new Packet1Login(this.mc.session.username, mc.session.getProfile().getId(), NFC.rawVersion));
		}
	...
	public void failConnect(String message, String reason) {
		mc.displayGuiScreen(new GuiConnectFailed(reason, message,
				new Object[] { message }));
		disconnect(reason);
	}
	...
	public void disconnect(String reason) {
		disconnected = true;
		netManager.wakeThreads();
		netManager.networkShutdown(reason, new Object[0]);
	}
	...
	public void handleMobSpawn(Packet24MobSpawn packet24mobspawn) {
		...
		EntityLiving entityliving = (EntityLiving) EntityList.createEntity(
				packet24mobspawn.type, mc.theWorld);
	...
	public void handleSpawnPosition(Packet6SpawnPosition packet6spawnposition) {
		mc.thePlayer.setPlayerSpawnCoordinate(new ChunkCoordinates(
				packet6spawnposition.xPosition, packet6spawnposition.yPosition,
				packet6spawnposition.zPosition));
		mc.theWorld.getWorldInfo().setSpawn(packet6spawnposition.xPosition,
				packet6spawnposition.yPosition, packet6spawnposition.zPosition);
	}
	...
	public void func_9448_a(Packet9Respawn packet9respawn) {
		if (packet9respawn.field_28048_a != mc.thePlayer.dimension) {
			field_1210_g = false;
			worldClient = new WorldClient(this, worldClient.getWorldInfo()
					.getRandomSeed(), packet9respawn.field_28048_a);
			worldClient.multiplayerWorld = true;
			mc.changeWorld1(worldClient);
	...
	public void func_12245_a(Packet60Explosion packet60explosion) {
		Explosion explosion = new Explosion(mc.theWorld, null,
				packet60explosion.explosionX, packet60explosion.explosionY,
				packet60explosion.explosionZ, packet60explosion.explosionSize);
	...
	public void func_20087_a(Packet100OpenWindow packet100openwindow) {
		if (packet100openwindow.inventoryType == 0) {
			InventoryBasic inventorybasic = new InventoryBasic(
					packet100openwindow.windowTitle,
					packet100openwindow.slotsCount);
			...
			mc.thePlayer.craftingInventory.windowId = packet100openwindow.windowId;
		} else if (packet100openwindow.inventoryType == 9) {
				TileEntityBrickOven tileentityfurnace = new TileEntityBrickOven();
				mc.thePlayer.displayGUIBrickOven(tileentityfurnace);
				mc.thePlayer.craftingInventory.windowId = packet100openwindow.windowId;
			...
			mc.thePlayer.craftingInventory.windowId = packet100openwindow.windowId;
		} else if (packet100openwindow.inventoryType == 8) {
			TileEntityBookshelf ent = new TileEntityBookshelf();
			mc.thePlayer.displayGUIBookshelf(ent);
			mc.thePlayer.craftingInventory.windowId = packet100openwindow.windowId;
		}
	}
	...
	public void func_20088_a(Packet103SetSlot packet103setslot) {
		...
		} else if (packet103setslot.windowId == 0
				&& packet103setslot.itemSlot >= 36
				&& packet103setslot.itemSlot < 45) {
			ItemStack itemstack = mc.thePlayer.inventorySlots.getSlot(
					packet103setslot.itemSlot).getStack();
			if (packet103setslot.myItemStack != null
					&& (itemstack == null || itemstack.stackSize < packet103setslot.myItemStack.stackSize)) {
				packet103setslot.myItemStack.animationsToGo = 5;
			}
			mc.thePlayer.inventorySlots.putStackInSlot(
					packet103setslot.itemSlot, packet103setslot.myItemStack);
		} else if (packet103setslot.windowId == mc.thePlayer.craftingInventory.windowId) {
			mc.thePlayer.craftingInventory.putStackInSlot(
					packet103setslot.itemSlot, packet103setslot.myItemStack);
		}
	}
		...
		if (container != null) {
			if (packet106transaction.field_20030_c) {
				...
				addToSendQueue(new Packet106Transaction(
						packet106transaction.windowId,
						packet106transaction.field_20028_b, true));
			}
		}
	}
	...
	public void func_20094_a(Packet104WindowItems packet104windowitems) {
		if (packet104windowitems.windowId == 0) {
			mc.thePlayer.inventorySlots
					.putStacksInSlots(packet104windowitems.itemStack);
		} else if (packet104windowitems.windowId == mc.thePlayer.craftingInventory.windowId) {
			mc.thePlayer.craftingInventory
					.putStacksInSlots(packet104windowitems.itemStack);
		}
	}

	public void handleSignUpdate(Packet130UpdateSign packet130updatesign) {
		if (mc.theWorld.blockExists(packet130updatesign.xPosition,
	...
	public void func_20090_a(
			Packet105UpdateProgressbar packet105updateprogressbar) {
		registerPacket(packet105updateprogressbar);
		if (mc.thePlayer.craftingInventory != null
				&& mc.thePlayer.craftingInventory.windowId == packet105updateprogressbar.windowId) {
			mc.thePlayer.craftingInventory.func_20112_a(
					packet105updateprogressbar.progressBar,
					packet105updateprogressbar.progressBarValue);
		}
	}

	public void handleBook(Packet300CustomBook pack) {
		if (mc.thePlayer.isDead)
			return;
		if (pack.compound != null) {
			mc.thePlayer.inventory.mainInventory[mc.thePlayer.inventory.currentItem].stackTagCompound = pack.compound;
		}
		mc.displayGuiScreen(new GuiScreenBook(mc.thePlayer,
				mc.thePlayer.inventory.getCurrentItem(), !pack.signbook));
	}
	
	public void handleScore(Packet301Score score)
	{
		if(getEntityByID(score.id) != null && getEntityByID(score.id) instanceof EntityPlayer)
		{
			((EntityPlayer)getEntityByID(score.id)).setScore(score.score);
		}
	}
	
	public void handleList(Packet302PlayerList list)
	{
		playerList = new ArrayList<ListPlayer>();
		for(int i = 0; i < list.getPacketSize(); i++)
		{
			playerList.add(new ListPlayer(list.usernames[i], list.scores[i]));
		}
	}
	...
	public void handlePlayerInventory(
		...
		if (entity != null) {
			entity.outfitWithItem(packet5playerinventory.slot,
	...
	public void handleNotePlay(Packet54PlayNoteBlock packet54playnoteblock) {
		mc.theWorld.playNoteAt(packet54playnoteblock.xLocation,
	...
	public void func_25118_a(Packet70Bed packet70bed) {
		...
		if (i >= 0 && i < Packet70Bed.field_25020_a.length
				&& Packet70Bed.field_25020_a[i] != null) {
	...
	public void func_28116_a(Packet131MapData packet131mapdata) {
		if (packet131mapdata.field_28055_a == Item.mapItem.shiftedIndex) {
			ItemMap.func_28013_a(packet131mapdata.field_28054_b, mc.theWorld)
					.func_28171_a(packet131mapdata.field_28056_c);
		} else {
			System.out.println((new StringBuilder()).append("Unknown itemid: ")
					.append(packet131mapdata.field_28054_b).toString());
		}
	}

	public void func_28115_a(Packet61DoorChange packet61doorchange) {
		mc.theWorld.func_28106_e(packet61doorchange.field_28050_a,
	...
	public void func_27245_a(Packet200Statistic packet200statistic) {
		((EntityClientPlayerMP) mc.thePlayer).func_27027_b(
				StatList.func_27361_a(packet200statistic.field_27052_a),
				packet200statistic.field_27051_b);
	}
	...
	public List<ListPlayer> playerList;
	Random rand;
}
