...
public abstract class BlockFluid extends Block {
	...
	public int colorMultiplier(IBlockAccess iblockaccess, int i, int j, int k) {
		if(iblockaccess.getBlockId(i, j, k) == 200 || iblockaccess.getBlockId(i, j, k) == 201) return 0x5C3000;
		return 0xffffff;
	}
	...
	protected int getEffectiveFlowDecay(IBlockAccess iblockaccess, int i,
			int j, int k) {
	...
	private Vec3D getFlowVector(IBlockAccess iblockaccess, int i, int j, int k) {
		...
		for (int i1 = 0; i1 < 4; i1++) {
			...
			if (i2 < 0) {
				...
				if (i2 >= 0) {
					int j2 = i2 - (l - 8);
					vec3d = vec3d.addVector((j1 - i) * j2, (k1 - j) * j2,
							(l1 - k) * j2);
				}
				continue;
			}
			if (i2 >= 0) {
				int k2 = i2 - l;
				vec3d = vec3d.addVector((j1 - i) * k2, (k1 - j) * k2, (l1 - k)
						* k2);
			}
		}
	...
	public static double func_293_a(IBlockAccess iblockaccess, int i, int j,
			int k, Material material, int id) {
		Vec3D vec3d = null;
		...
		if (material == Material.oil) {
			vec3d = ((BlockFluid) NFC.Oil).getFlowVector(
					iblockaccess, i, j, k);
		}
		if (material == Material.lava) {
			vec3d = ((BlockFluid) Block.lavaMoving).getFlowVector(iblockaccess,
					i, j, k);
		}
