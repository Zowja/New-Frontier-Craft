...
//            RenderHelper, mod_NewFrontier, MathHelper, GuiChat, 
//            ChatLine, Tessellator, BlockPortal, RenderItem, 
//            StringTranslate
...
public class GuiIngame extends GuiScreen {
	...
	public void renderGameOverlay(float f, boolean flag, int i, int j) {
		...
		if (!mc.gameSettings.thirdPersonView && Minecraft.isFancyGraphicsEnabled()) {
		...
		drawTexturedModalRect((k / 2 - 91 - 1) + inventoryplayer.currentItem
				* 20, l - 22 - 1, 0, 22, 24, 22);
		GL11.glBindTexture(3553 /* GL_TEXTURE_2D */,
		...
		if (mc.playerController.shouldDrawHUD()) {
			...
			if (mc.thePlayer.isInsideOfMaterial(Material.water) || mc.thePlayer.isInsideOfMaterial(Material.oil)) {
				int k2 = (int) Math
						.ceil(((double) (mc.thePlayer.air - 2) * 10D) / 300D);
				int l3 = (int) Math
						.ceil(((double) mc.thePlayer.air * 10D) / 300D) - k2;
				for (int l5 = 0; l5 < k2 + l3; l5++) {
					if (l5 < k2) {
						drawTexturedModalRect((k / 2 - 91) + l5 * 8,
								l - 32 - 9, 16, 18, 9, 9);
					} else {
						drawTexturedModalRect((k / 2 - 91) + l5 * 8,
								l - 32 - 9, 25, 18, 9, 9);
		...
		if (mc.gameSettings.showDebugInfo) {
			...
			fontrenderer.drawStringWithShadow((new StringBuilder()).append("(")
					.append(mc.debug).append(")").toString(), 2, 12, 0xffffff);
			fontrenderer
					.drawStringWithShadow(mc.func_6241_m(), 2, 22, 0xffffff);
			fontrenderer
					.drawStringWithShadow(mc.func_6262_n(), 2, 32, 0xffffff);
			fontrenderer
					.drawStringWithShadow(mc.func_6245_o(), 2, 42, 0xffffff);
			fontrenderer.drawStringWithShadow(mc.func_21002_o(), 2, 52,
					0xffffff);
			long l2 = Runtime.getRuntime().maxMemory();
			long l4 = Runtime.getRuntime().totalMemory();
			long l6 = Runtime.getRuntime().freeMemory();
			long l7 = l4 - l6;
			String s = (new StringBuilder()).append("Used memory: ")
					.append((l7 * 100L) / l2).append("% (")
					.append(l7 / 1024L / 1024L).append("MB) of ")
					.append(l2 / 1024L / 1024L).append("MB").toString();
			drawString(fontrenderer, s, k - fontrenderer.getStringWidth(s) - 2,
					2, 0xe0e0e0);
			s = (new StringBuilder()).append("Allocated memory: ")
					.append((l4 * 100L) / l2).append("% (")
			...
			fontrenderer.drawStringWithShadow(
					(new StringBuilder()).append(mc.theWorld.getRandomSeed())
							.toString(), 2, 100, 0xffffff);
		}
		if (recordPlayingUpFor > 0) {
			...
			if (j3 > 0) {
				...
				fontrenderer.drawString(recordPlaying,
						-fontrenderer.getStringWidth(recordPlaying) / 2, -4, k4
								+ (j3 << 24));
				GL11.glDisable(3042 /* GL_BLEND */);
				GL11.glPopMatrix();
			}
		...
		for (int i5 = 0; i5 < chatMessageList.size() && i5 < byte0; i5++) {
			if (((ChatLine) chatMessageList.get(i5)).updateCounter >= 200
					&& !flag2) {
				continue;
			}
			...
			if (j6 > 0) {
				...
				GL11.glEnable(3042 /* GL_BLEND */);
				fontrenderer.drawStringWithShadow(s1, byte1, k6,
						0xffffff + (j6 << 24));
			}
		}
		
		if(mc.isMultiplayerWorld() && Keyboard.isKeyDown(Keyboard.KEY_TAB))
		{
			renderPlayerList();
		}
	...
	private void renderTelescopeVision(int i, int j) {
		...
		tessellator.draw();
		GL11.glBindTexture(3553 /* GL_TEXTURE_2D */,
				mc.renderEngine.getTexture("/NFC/glass.png"));
		tessellator.startDrawingQuads();
		tessellator.addVertexWithUV((i/2)-(j/2), j, -90D, 0.0D, 1.0D);
		tessellator.addVertexWithUV((i/2)+(j/2), j, -90D, 1.0D, 1.0D);
		tessellator.addVertexWithUV((i/2)+(j/2), 0.0D, -90D, 1.0D, 0.0D);
	...
	public void addChatMessage(String s) {
		...
		for (; mc.fontRenderer.getStringWidth(s) > 320; s = s.substring(i)) {
			for (i = 1; i < s.length()
					&& mc.fontRenderer.getStringWidth(s.substring(0, i + 1)) <= 320; i++) {
			}
			addChatMessage(s.substring(0, i));
		}

		chatMessageList.add(0, new ChatLine(s));
		for (; chatMessageList.size() > 50; chatMessageList
				.remove(chatMessageList.size() - 1)) {
		}
	}

	public void setRecordPlayingMessage(String s) {
		recordPlaying = (new StringBuilder()).append("Now playing: ").append(s)
				.toString();
	...
	public void renderPlayerList()
	{
        ...
        List<ListPlayer> list;
        if(((EntityClientPlayerMP)mc.thePlayer).sendQueue.playerList != null)
        {
        	list = ((EntityClientPlayerMP)mc.thePlayer).sendQueue.playerList;
        } else { return;}
        ...
        int j6 = (k - k4 * k5) / 2;
        int height = -180;
        if(scaledresolution.scaleFactor == 3)
        {
        	height = -195;
        }
        drawRect(j6 - 1, height - 1, j6 + k5 * k4, height + 9 * i4, 0x80000000);
        for(int k7 = 0; k7 < j3; k7++)
        {
            int i8 = j6 + (k7 % k4) * k5;
            int l8 = height + (k7 / k4) * 9;
            ...
            ListPlayer player = list.get(k7);
            mc.fontRenderer.drawStringWithShadow(player.username, i8, l8, 0xffffff);
            mc.fontRenderer.drawStringWithShadow("Score:" + "\u00A7e" + " " + player.score, i8 + 80, l8, 0xffffff);
            zLevel += 100F;
            zLevel -= 100F;
        }
	}
