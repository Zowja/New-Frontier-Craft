...
public class SoundManager {
	...
	public void loadSoundSettings(GameSettings gamesettings) {
		...
		if (!loaded
				&& (gamesettings == null || gamesettings.soundVolume != 0.0F || gamesettings.musicVolume != 0.0F)) {
	...
	public void onSoundOptionsChanged() {
		...
		if (loaded) {
			if (options.musicVolume == 0.0F) {
				...
				sndSystem.setVolume("Menu", options.musicVolume);
			}
		}
	}
	...
	public void playMenuMusic() {
		...
		SoundPoolEntry soundpoolentry = soundPoolStreaming.getRandomSoundFromSoundPool("background");
		if (soundpoolentry != null) {
			sndSystem.backgroundMusic("Menu", soundpoolentry.soundUrl,
					soundpoolentry.soundName, false);
			sndSystem.setVolume("Menu", options.musicVolume);
			sndSystem.play("Menu");
		}
	...
	public void func_338_a(EntityLiving entityliving, float f) {
		...
		if (entityliving == null) {
			...
			double d = entityliving.prevPosX
					+ (entityliving.posX - entityliving.prevPosX) * (double) f;
			double d1 = entityliving.prevPosY
					+ (entityliving.posY - entityliving.prevPosY) * (double) f;
			double d2 = entityliving.prevPosZ
					+ (entityliving.posZ - entityliving.prevPosZ) * (double) f;
	...
	public void playStreaming(String s, float f, float f1, float f2, float f3,
			float f4) {
		if (!loaded || options.soundVolume == 0.0F) {
			return;
		}
		...
		SoundPoolEntry soundpoolentry = soundPoolStreaming
				.getRandomSoundFromSoundPool(s);
		if (soundpoolentry != null && f3 > 0.0F) {
			if (sndSystem.playing("BgMusic")) {
				sndSystem.stop("BgMusic");
			}
			float f5 = 16F;
			sndSystem.newStreamingSource(true, s1, soundpoolentry.soundUrl,
					soundpoolentry.soundName, false, f, f1, f2, 2, f5 * 4F);
			System.out.println(options.soundVolume);
			sndSystem.setVolume(s1, 0.5F * options.soundVolume);
			sndSystem.play(s1);
		}
	}

	public void playSound(String s, float f, float f1, float f2, float f3,
			float f4) {
		if (!loaded || options.soundVolume == 0.0F) {
			return;
		}
		...
		if (soundpoolentry != null && f3 > 0.0F) {
			...
			String s1 = (new StringBuilder()).append("sound_")
					.append(field_587_e).toString();
			float f5 = 16F;
			if (f3 > 1.0F) {
				f5 *= f3;
			}
			sndSystem.newSource(f3 > 1.0F, s1, soundpoolentry.soundUrl,
					soundpoolentry.soundName, false, f, f1, f2, 2, f5);
	...
	public void playSoundFX(String s, float f, float f1) {
		...
		if (soundpoolentry != null) {
			...
			String s1 = (new StringBuilder()).append("sound_")
					.append(field_587_e).toString();
			sndSystem.newSource(false, s1, soundpoolentry.soundUrl,
