...
public class EntityLightningBolt extends EntityWeatherEffect {
	...
	public EntityLightningBolt(World world, double d, double d1, double d2) {
		...
		if (world.difficultySetting >= 2
				&& world.doChunksNearChunkExist(MathHelper.floor_double(d),
						MathHelper.floor_double(d1),
						MathHelper.floor_double(d2), 10)) {
			int i = MathHelper.floor_double(d);
			int k = MathHelper.floor_double(d1);
			int i1 = MathHelper.floor_double(d2);
			if (world.getBlockId(i, k, i1) == 0
					&& Block.fire.canPlaceBlockAt(world, i, k, i1)) {
			...
			for (int j = 0; j < 4; j++) {
				...
				if (world.getBlockId(l, j1, k1) == 0
						&& Block.fire.canPlaceBlockAt(world, l, j1, k1)) {
