...
public abstract class EntityPlayer extends EntityLiving {
	...
	public EntityPlayer(World world) {
		...
		setLocationAndAngles((double) chunkcoordinates.x + 0.5D,
				chunkcoordinates.y + 1, (double) chunkcoordinates.z + 0.5D,
				0.0F, 0.0F);
		health = 20;
	...
	public void onUpdate() {
		...
		super.onUpdate();
		if (!worldObj.multiplayerWorld && craftingInventory != null
				&& !craftingInventory.isUsableByPlayer(this)) {
			closeScreen();
			craftingInventory = inventorySlots;
		}
	...
	public void displayGUIBook(ItemStack par1ItemStack, boolean par1) {
	}
	...
	public void onLivingUpdate() {
		if (worldObj.difficultySetting == 0 && health < 20
	...
	public void onDeath(Entity entity) {
		...
		if (username.equals("Notch") && !worldObj.multiplayerWorld) {
			dropPlayerItemWithRandomChoice(new ItemStack(Item.appleRed, 1),
					true);
		} else
		if(username.equals("Zowja") || username.equals("0n1") && !worldObj.multiplayerWorld)
		{
			dropPlayerItemWithRandomChoice(new ItemStack(NFC.cheese), true);
		}
		inventory.dropAllItems();
		if (entity != null) {
			motionX = -MathHelper
					.cos(((attackedAtYaw + rotationYaw) * 3.141593F) / 180F) * 0.1F;
			motionZ = -MathHelper
					.sin(((attackedAtYaw + rotationYaw) * 3.141593F) / 180F) * 0.1F;
		} else {
	...
	public void addToPlayerScore(Entity entity) {
		if(!worldObj.multiplayerWorld)
		{
		score += EntityScore.getScore(entity, this);
		}
	...
	public float getCurrentPlayerStrVsBlock(Block block) {
		...
		if (isInsideOfMaterial(Material.oil)) {
			f /= 5F;
		}
		if (!onGround) {
			f /= 5F;
	...
	public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		if (sleeping) {
			bedChunkCoordinates = new ChunkCoordinates(
					MathHelper.floor_double(posX),
					MathHelper.floor_double(posY),
					MathHelper.floor_double(posZ));
			wakeUpPlayer(true, true, false);
		}
		if (nbttagcompound.hasKey("SpawnX") && nbttagcompound.hasKey("SpawnY")
				&& nbttagcompound.hasKey("SpawnZ")) {
			playerSpawnCoordinate = new ChunkCoordinates(
	...
	public void displayGUIBookshelf(TileEntityBookshelf iinventory) {
	}
	...
	protected void alertWolves(EntityLiving entityliving, boolean flag) {
		...
		if (entityliving instanceof EntityWolf) {
			...
			if (entitywolf.isWolfTamed()
					&& username.equals(entitywolf.getWolfOwner())) {
				return;
			}
		}
		...
		do {
			...
			if (entitywolf1.isWolfTamed() && entitywolf1.getTarget() == null
					&& username.equals(entitywolf1.getWolfOwner())
					&& (!flag || !entitywolf1.isWolfSitting())) {
	...
	public void displayGUIBrickOven(TileEntityBrickOven tileentityBrickOVen) {
	}
	...
	public void wakeUpPlayer(boolean flag, boolean flag1, boolean flag2) {
		...
		if (chunkcoordinates != null
				&& worldObj.getBlockId(chunkcoordinates.x, chunkcoordinates.y,
						chunkcoordinates.z) == Block.blockBed.blockID) {
			BlockBed.setBedOccupied(worldObj, chunkcoordinates.x,
					chunkcoordinates.y, chunkcoordinates.z, false);
			ChunkCoordinates chunkcoordinates2 = BlockBed
					.getNearestEmptyChunkCoordinates(worldObj,
							chunkcoordinates.x, chunkcoordinates.y,
							chunkcoordinates.z, 0);
			if (chunkcoordinates2 == null) {
				chunkcoordinates2 = new ChunkCoordinates(chunkcoordinates.x,
						chunkcoordinates.y + 1, chunkcoordinates.z);
			}
			setPosition((float) chunkcoordinates2.x + 0.5F,
					(float) chunkcoordinates2.y + yOffset + 0.1F,
	...
	private boolean isInBed() {
		return worldObj.getBlockId(bedChunkCoordinates.x,
				bedChunkCoordinates.y, bedChunkCoordinates.z) == Block.blockBed.blockID;
	}

	public static ChunkCoordinates func_25060_a(World world,
			ChunkCoordinates chunkcoordinates) {
		IChunkProvider ichunkprovider = world.getIChunkProvider();
		ichunkprovider.prepareChunk(chunkcoordinates.x - 3 >> 4,
				chunkcoordinates.z - 3 >> 4);
		ichunkprovider.prepareChunk(chunkcoordinates.x + 3 >> 4,
				chunkcoordinates.z - 3 >> 4);
		ichunkprovider.prepareChunk(chunkcoordinates.x - 3 >> 4,
				chunkcoordinates.z + 3 >> 4);
		ichunkprovider.prepareChunk(chunkcoordinates.x + 3 >> 4,
				chunkcoordinates.z + 3 >> 4);
		if (world.getBlockId(chunkcoordinates.x, chunkcoordinates.y,
				chunkcoordinates.z) != Block.blockBed.blockID) {
			return null;
		} else {
			ChunkCoordinates chunkcoordinates1 = BlockBed
					.getNearestEmptyChunkCoordinates(world, chunkcoordinates.x,
	...
	public float getBedOrientationInDegrees() {
		if (bedChunkCoordinates != null) {
			int i = worldObj.getBlockMetadata(bedChunkCoordinates.x,
					bedChunkCoordinates.y, bedChunkCoordinates.z);
	...
	private void addMovementStat(double d, double d1, double d2) {
		...
		if (isInsideOfMaterial(Material.water)) {
			int i = Math.round(MathHelper
					.sqrt_double(d * d + d1 * d1 + d2 * d2) * 100F);
			if (i > 0) {
		...
		} else if (isInsideOfMaterial(Material.oil)) {
			int i = Math.round(MathHelper
					.sqrt_double(d * d + d1 * d1 + d2 * d2) * 100F);
			if (i > 0) {
				addStat(StatList.distanceDoveStat, i);
	...
	public int getItemIcon(ItemStack itemstack) {
		...
		if (itemstack.itemID == Item.fishingRod.shiftedIndex
				&& fishEntity != null) {
	...
	public boolean modelAlex = false;
